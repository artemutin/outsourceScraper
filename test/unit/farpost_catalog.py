...
from outsourceScraper.FarpostDictionaryScraper import *
...
class FarpostCatalog(unittest.TestCase):
    ...
    def testSpecificCatalogPage(self):
        page = CatalogPage('http://www.vl.ru/taxi-transport-delivery/transportation/goods-traffic?page=19')
        scraper = FarpostDictionaryScraper(page, True)
        print(list(scraper.ads))


    def testfullCatalogScrape(self):
        logging.info('IN test')
        # ask to parse all catalogue
        results = scrape_catalog('vl', category='Бухгалтерия', city='Владивосток', num_pages=2)

        # ALERT: this test is unstable by definition
        self.assertEqual(len(results), 60)
        results = scrape_catalog('vl', category='Бухгалтерия', city='Хабаровск', num_pages=2)
